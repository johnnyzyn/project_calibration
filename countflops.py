...
parser.add_argument("--archi_end", type=int, default=32768, help="archi number to end")
    ...
    if args.api_type == 'tss':
        ...
        with open("error_log.txt", "a") as error_log:
            for i in missing_ids_list:
                archi_num = i
                try:
                    result = subprocess.run(["python", "main_countflops.py", "--archi_num", str(archi_num), "--csv_file", args.csv_file, "--image_dataset", args.image_dataset,"--post_temp",args.post_temp
    ...
    if args.api_type == 'tss':
        ...
        with open("error_log.txt", "a") as error_log:
            ...
            for archi_num in range(args.archi_start, args.archi_end):
                try:
                    result = subprocess.run(["python", "main_countflops.py"
                                            , "--archi_num", str(archi_num)
                                            , "--csv_file", args.csv_file
                                            ,"--post_temp",args.post_temp
                                            ,"--api_type",args.api_type
                    ...
                    error_log.write(error_msg)
                    continue
# python countflops.py --csv_file sss_flops.csv --api_type sss
